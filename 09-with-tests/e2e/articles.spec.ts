...
test.describe('Article CRUD Operations (Authenticated)', () => {
  ...
  test('should show New Article button when authenticated', async ({ page }) => {
    await page.goto('/');
  ...
  test('should create a new article', async ({ page }) => {
    ...
    const contentTextarea = page.locator('textarea[name="content"]').or(page.locator('.w-md-editor-text-input'));
    ...
    await contentTextarea.first().fill('This is a test article content created by Playwright E2E test.');
  ...
  test('should update an existing article', async ({ page }) => {
    ...
    const editContentTextarea = page.locator('textarea[name="content"]').or(page.locator('.w-md-editor-text-input'));
    ...
    await editContentTextarea.first().fill('This content has been updated by Playwright test.');
    ...
    const updatedTitleElement = page.locator('h1').filter({ hasText: updatedTitle });
  ...
  test('should prevent creating article with empty content', async ({ page }) => {
    await page.goto('/wiki/edit/new');
  ...
  test('should display article content on detail page', async ({ page }) => {
    ...
    await titleInput.fill(uniqueTitle);
    await contentTextarea.first().fill('Testing detail page display with unique content.');
    ...
    const content = page.locator('text=Testing detail page display with unique content');
    await expect(content).toBeVisible();
  });
  ...
  test('should display AI-generated summary for articles', async ({ page }) => {
    ...
    await titleInput.fill(uniqueTitle);
    await contentTextarea.first().fill('This is a comprehensive article about testing. It covers various aspects of E2E testing with Playwright and how to write effective tests.');
  ...
  test('should allow editing own articles', async ({ page }) => {
    ...
    await page.locator('textarea[name="content"]').or(page.locator('.w-md-editor-text-input')).first().fill('Original content');
